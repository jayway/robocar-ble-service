...
(function() {
  var fs, ioctl, uinput, uinputStructs, virtual_gamepad;
        ...
        return function(err, fd) {
          ...
          if (err) {
            ...
            return fs.write(_this.fd, uidev_buffer, 0, uidev_buffer.length, null, function(err) {
              var error1;
              if (err) {
                console.log("Error on init gamepad write:\n");
                return error(err);
              } else {
                try {
                  ...
                  _this.fd = void 0;
                  if (retry < 5) {
                    console.log("Retry to create gamepad");
                    return _this.connect(callback, error, retry + 1);
                  } else {
                    console.log("Gave up on creating device");
                    return error(err);
                  }
                }
              }
    ...
    virtual_gamepad.prototype.sendEvent = function(event, error) {
      ...
      if (this.fd) {
        ...
        try {
          ...
          err = error1;
          console.log("Error on writing ev_buffer");
          throw err;
        }
        try {
          ...
          err = error2;
          console.log("Error on writing ev_end_buffer");
          throw err;
        }
      }
    };
